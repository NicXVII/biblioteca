...
if (true) {
    ...
    if (!$db) {
        ...
        if (isset($data['id'])) {
            ...
            $queryIdLibro = "SELECT tprenotazionecarta.idCarta FROM tprenotazionecarta
            WHERE tprenotazionecarta.idPrenotazione = ?";
            $stmt = mysqli_prepare($db, $queryIdLibro);

            mysqli_stmt_bind_param($stmt, "i", $id);
            mysqli_stmt_execute($stmt);
            $resultIdLibro = mysqli_stmt_get_result($stmt);
            $idLibro = mysqli_fetch_array($resultIdLibro)['idCarta'];

            // Debugging: Check if the correct book ID is fetched
            //echo $idLibro;

            $queryControllo = "CALL checkPrestitoCarta($idLibro);";
            ...
            if ($count > 0) {
                $result = [
                    'success'    =>  false,
                    'message'   =>  'Carta is already borrowed',
                ];
