...
if (true) {
    ...
    if (!$db) {
        ...
        if (isset($data['id'])) {
            ...
            $queryIdLibro = "SELECT tprenotazioneenciclopedia.idVolume 
            FROM tprenotazioneenciclopedia
            WHERE tprenotazioneenciclopedia.idPrenotazione = ?";
            $stmt = mysqli_prepare($db, $queryIdLibro);

            mysqli_stmt_bind_param($stmt, "i", $id);
            mysqli_stmt_execute($stmt);
            $resultIdLibro = mysqli_stmt_get_result($stmt);
            $idLibro = mysqli_fetch_array($resultIdLibro)['idVolume'];

            // Debugging: Check if the correct book ID is fetched
            //echo $idLibro;

            $queryControllo = "CALL checkPrestitoVolume($idLibro);";
            ...
            if ($count > 0) {
                $result = [
                    'success'    =>  false,
                    'message'   =>  'Volume is already borrowed',
                ];
            ...
            $query = "UPDATE tprenotazioneenciclopedia
                      SET dataAccetazione = ?
